...
int main(int argc, char **argv, char **envp)
{
    ...
    shellcode_mem = mmap((void *) 0x1337000, 0x2000, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_PRIVATE|MAP_ANON, 0, 0);
    ...
    printf("Reading %#x bytes from stdin into %p.\n", 0x2000, shellcode_mem);

    shellcode_size = read(0, shellcode_mem, 0x2000);
    assert(shellcode_size > 0);

    printf("This challenge is about to remove write permissions from the first %d\n", 4096);
    printf("bytes of your shellcode. If you were counting on modifying those parts\n");
    printf("of your shellcode, you will have to develop a new strategy...\n");
    printf("\n");
    ...
    printf("This challenge requires that your shellcode does not have any 'syscall',\n");
    printf("'sysenter', or 'int' instructions. System calls are too dangerous!\n");
    printf("\n");
    printf("This filter works by scanning through the shellcode for the following\n");
    printf("byte sequences:\n");
    printf("\n");
    printf("- 0f05 for 'syscall'\n");
    printf("- 0f34 for 'sysenter'\n");
    printf("- 80cd for 'int'\n");
    printf("\n");
    printf("One way to evade this is to have your shellcode modify itself to insert\n");
    printf("the 'syscall' instructions at runtime...\n");
